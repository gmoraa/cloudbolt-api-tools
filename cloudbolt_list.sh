...
for (( x=0; x<$total; x++ ))
do
	status=`curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" "$1/api/v2/servers/${servers[$x]}/" -k | python -mjson.tool | grep "POWERON\|hostname"`
	echo $status | grep "POWERON" | awk '{print($2)}' | tr -d ',"'
